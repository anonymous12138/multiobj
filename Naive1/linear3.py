...
def get_training_sequence_linear(file, training_indep, training_dep, testing_indep, min_split=10,
    ...
    for i in range(len(lessismore['results_' + file + '/'])):
        if lessismore['results_' + file + '/'][i]:
            ...
            coef[i]=1
    new_training_dep = [coef[0]*each[0]+coef[1]*each[1] +coef[2]*each[2] for each in training_dep]
        ...
        for rep in range(20):
            ...
            while True:
                ...
                if len(previously_not_seen) == 0:
                    lives -= 1
                print("lives", lives, len(testing_indep))
            ...
            y_pred = pd.DataFrame([y_pred1, y_pred2,y_pred3]).T.values
            ...
            for i in range(len(lessismore['results_' + file + '/'])):
                if lessismore['results_' + file + '/'][i]:
                    ...
                    coef[i] = 1
            y_pred = [coef[0] * each[0] + coef[1] * each[1] + coef[2] * each[2] for each in y_pred]

            print("Tree Size:", model.get_depth(), model.get_n_leaves())
            pf_indexes = np.argsort(y_pred)
            if len(sizes) <= rep:
                pf_indexes = pf_indexes[-sizes[-1]:]
            else:

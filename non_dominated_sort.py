...
def non_dominated_sort(raw_dependents, lessismore, mins, maxs, domination="c"):
    if domination == "c":
        domination = continious_domination
    elif domination=="b":
        domination = binary_domination
...
def dom_rank(raw_dependents, lessismore, mins, maxs, domination=continious_domination):
    assert(len(raw_dependents[0]) == len(lessismore)), "Something is wrong"
    dependents = []
    for rd in raw_dependents:
        temp = []
    ...
    for first_count, f_individual in enumerate(dependents):
        ...
        num.append(dominating_fits[first_count])
    # smaller num[i] means higher/better rank
    return list(np.argsort(num))

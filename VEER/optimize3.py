...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        for rep in range(100):
            ...
            print("Tree Size1:",model.get_depth(),model.get_n_leaves())
            ...
            model.fit(pd.DataFrame(X_train_reduced), pd.DataFrame(y_train).iloc[:, 2])
            y3 = model.predict(pd.DataFrame(X_train_reduced))
            y_pred3 = model.predict(pd.DataFrame(X_test_reduced))
            print("Tree Size3:", model.get_depth(), model.get_n_leaves())

            merged = pd.DataFrame([y_pred1, y_pred2,y_pred3]).T
            ...
            if X_test.shape[0]<=1000:
                ...
                X_fake = X_test.sample(n=1000,replace=False)

            for i in range(3):
                model = DecisionTreeRegressor(criterion='mse',min_samples_split=split,min_impurity_decrease=impurity)
                model.fit(X_train, pd.DataFrame(y_train).iloc[:, i])
                y_pred.append(model.predict(X_fake))
            merged_pred = []
            for i, j, k in zip(y_pred[0], y_pred[1],y_pred[2]):
                merged_pred.append([i, j, k])
            layers = get_nd_layers(merged_pred, lessismore=lessismore['results_' + file + '/'])
            ly = list(np.concatenate(layers).flat)
            labels = [0] * len(ly)
            for i in range(len(layers)):
                for j in range(len(layers[i])):
                    labels[layers[i][j]] = i + 1
            ...
            feature_importance_combined.append(outter_model.feature_importances_)

            y_pred = outter_model.predict(X_test_reduced)
            print("Tree Size2:", outter_model.get_depth(), outter_model.get_n_leaves())
            pf_indexes = np.argsort(y_pred)[:size]
            current_pf = [y_valid[i] for i in pf_indexes]
            duration = time.time() - start
